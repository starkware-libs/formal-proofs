...
theorem auto_sound_u16_overflowing_sub
  ...
  : EnsuresRet mem σ fun κ τ =>
    ∃ μ ≤ κ, RcEnsures mem (rcBound F) μ (mem (σ.fp - 5)) (mem (τ.ap - 3))
      (spec_u16_overflowing_sub a b (mem (τ.ap - 2)) (mem (τ.ap - 1))) := by
  ...
  · -- a_ge_b == 0
    ...
    suffices auto_spec : auto_spec_u16_overflowing_sub a b _ _ by
      apply sound_u16_overflowing_sub; apply auto_spec
    use_only a_ge_b
    ...
    simp only [htv_a_minus_b, u16Limit]
    exact ret_sub
  -- a_ge_b != 0
