...
def spec_u64_overflowing_sub (a b ρ_overflow ρ_sub : F) : Prop :=
  ∀ na : ℕ, is_u64_of a na →
  ∀ nb : ℕ, is_u64_of b nb →
  ∃ n : ℕ, is_u64_of ρ_sub n ∧
    (ρ_overflow = 1 ∧ ρ_sub = a - b + u64Limit ∨
     ρ_overflow = 0 ∧ ρ_sub = a - b)

def auto_spec_u64_overflowing_sub (a b ρ_overflow ρ_sub : F) : Prop :=
  ∃ a_ge_b : F,
  ∃ a_minus_b : F, a_minus_b = a - b ∧
    ((a_ge_b = 0 ∧
      ∃ fixed_a_minus_b : F, fixed_a_minus_b = a_minus_b + u128Limit ∧
      IsRangeChecked (rcBound F) fixed_a_minus_b ∧
      ρ_overflow = 1 ∧ ρ_sub = a_minus_b + u64Limit)
    ∨
    (a_ge_b ≠ 0 ∧
      IsRangeChecked (rcBound F) a_minus_b ∧
      ρ_overflow = 0 ∧ ρ_sub = a_minus_b))

theorem sound_u64_overflowing_sub {a b ρ_overflow ρ_add : F}
  (h_auto : auto_spec_u64_overflowing_sub a b ρ_overflow ρ_add) :
  spec_u64_overflowing_sub a b ρ_overflow ρ_add := by
  ...
  · have hm_u128 := lt_of_lt_of_le h₁ h_rcBound
    use m + u64Limit - u128Limit
    constructor
    · constructor
      · exact add_sub_lt u64Limit_pos hm_u128
      have hm₂ : u128Limit ≤ m + u64Limit := by
        rw [add_comm, ← (add_left_inj (u64Limit : F)), add_assoc] at h₂
        conv at h₂ => lhs; rhs; rw [add_comm]
        rw [← add_sub_assoc, ← cast_add, ← cast_sub, ← cast_add, ← cast_add] at h₂
        rw [PRIME.nat_coe_field_inj (add_lt_PRIME_of_lt hm_u128 u64Limit_lt_u128Limit) _ h₂.symm]
        · exact le_self_add
        · apply add_lt_PRIME_of_le (le_refl _)
          have htemp₁ : u64Limit + na - nb ≤ u64Limit + na := by exact Nat.sub_le _ _
          have htemp₂ : u64Limit + na ≤ u64Limit + u64Limit := le_of_lt ((add_lt_add_iff_left _).mpr hna)
          have htemp₃ : u64Limit + u64Limit ≤ u128Limit := by unfold u64Limit u128Limit; norm_num1
  ...
  · use m
    ...
    · constructor
      · apply lt_of_le_of_lt _ hna
        ...
        rw [PRIME.nat_coe_field_inj _ (lt_trans hna u64Limit_lt_PRIME) h₂.symm]
        exact add_lt_PRIME_of_lt (lt_of_lt_of_le h₁ h_rcBound)
          (lt_trans hnb u64Limit_lt_u128Limit)
      exact h₂
