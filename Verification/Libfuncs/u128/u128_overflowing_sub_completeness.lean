...
open vm_u128_overflowing_sub_code
open u128_overflowing_sub_completeness

variable (mem : Mrel → Mrel) (σ : VmRegisterState)


theorem complete_u128_overflowing_sub_NoOverflow_from_spec
    -- arguments
    (range_check a_minus_b : ℤ)
    -- code is in memory at σ.pc + start offset
    (hmem : ProgAt mem vm_u128_overflowing_sub_code σ.pc)
    -- input arguments on the stack
    (hin_fp : σ.fp = σ.ap)
    (htv_range_check : rc range_check = mem (exec (σ.ap - 5)))
    (htv_a_minus_b : val a_minus_b = mem (exec (σ.ap + 1)))
    (h_spec: ∃ (ρ_branch_id ρ_a_minus_b : ℤ),
               auto_spec_u128_overflowing_sub_NoOverflow range_check a_minus_b ρ_branch_id ρ_a_minus_b)
  -- conclusion
  ...
  rcases h_spec with ⟨ρ_branch_id, ρ_a_minus_b, h_rc_a_minus_b, h_ρ_branch_id, h_ρ_a_minus_b⟩
  let exec_vals :=
      fun (i : ℤ) =>
        match (i - (σ.ap + 2)) with
        | 0 => val 0
...
theorem complete_u128_overflowing_sub_from_spec
    -- arguments
    (range_check a b : ℤ)
    -- code is in memory at σ.pc + start offset
    (hmem : ProgAt mem vm_u128_overflowing_sub_code σ.pc)
    ...
    (h_spec: ∃ (ρ_branch_id ρ_a_minus_b : ℤ),
               auto_spec_u128_overflowing_sub range_check a b ρ_branch_id ρ_a_minus_b)
  -- conclusion
  : ∃ loc : LocalAssignment σ.ap range_check,
    ...
    Returns PRIME (Assign mem loc) σ (fun κ τ =>
      ...
      Assign mem loc (exec (τ.ap - 3)) = rc (range_check + loc.rc_num)) := by
  rcases h_spec with ⟨ρ_branch_id, ρ_a_minus_b, orig_range_check, h_orig_range_check, a_ge_b, a_minus_b, h_a_minus_b, h_spec|h_spec⟩
  · rcases h_spec with ⟨hc_a_ge_b, wrapping_a_minus_b, h_wrapping_a_minus_b, h_rc_wrapping_a_minus_b, h_ρ_branch_id, h_ρ_a_minus_b⟩
    let exec_vals :=
        fun (i : ℤ) =>
          match (i - σ.ap) with
          | 0 => val a_ge_b
          | 1 => val a_minus_b
          | 2 => val wrapping_a_minus_b
          | 3 => rc (range_check + 1)
          | 4 => val ρ_branch_id
          | 5 => val ρ_a_minus_b
          | _ => val 0
    ...
    · try simp only [neg_clip_checked', ←Int.sub_eq_add_neg]
      ...
      try dsimp [exec_vals, rc_vals]
      try simp only [add_sub_add_comm, add_sub_right_comm, sub_add_cancel_left, sub_self] ; try norm_num1
      rw [Int.eq_sub_emod_iff_add_emod_eq] at h_a_minus_b
      dsimp [Mrel.Equiv]
      simp only [h_a_minus_b]
    ...
    · try simp only [neg_clip_checked', ←Int.sub_eq_add_neg]
      ...
      try simp only [add_sub_add_comm, add_sub_right_comm, sub_add_cancel_left, sub_self] ; try norm_num1
      dsimp [Mrel.Equiv]
      simp only [u128_limit] at h_wrapping_a_minus_b
      simp only [h_wrapping_a_minus_b]
    ...
    try ring_nf
    try rfl
  rcases h_spec with ⟨hc_a_ge_b, h_block_NoOverflow⟩
  let exec_vals :=
      fun (i : ℤ) =>
        match (i - σ.ap) with
        | 0 => val a_ge_b
  ...
  have h_a_minus_b_exec_pos := assign_exec_pos mem loc₀ (σ.ap + 1)
    (by use (Int.le_add_of_nonneg_right (by norm_num1)) ; apply Int.add_lt_add_left ; norm_num1)
  have h_a_minus_b_ap : val ↑a_minus_b = Assign mem loc₀ (exec (σ.ap + 1)) := by
    simp only [h_a_minus_b_exec_pos] ; dsimp [exec_vals] ; ring_nf ; rfl
  have h_NoOverflow := complete_u128_overflowing_sub_NoOverflow_from_spec
    (Assign mem loc₀) σ range_check a_minus_b hmem hin_fp h_range_check_ap h_a_minus_b_ap ⟨ρ_branch_id, ρ_a_minus_b, h_block_NoOverflow⟩
