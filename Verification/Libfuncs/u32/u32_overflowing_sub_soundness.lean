...
theorem auto_sound_u32_overflowing_sub
    ...
    (hmem : MemAt mem u32_overflowing_sub_code σ.pc)
  ...
  : EnsuresRet mem σ fun κ τ =>
    ∃ μ ≤ κ, RcEnsures mem (rcBound F) μ (mem (σ.fp - 5)) (mem (τ.ap - 3))
      (spec_u32_overflowing_sub a b (mem (τ.ap - 2)) (mem (τ.ap - 1))) := by
  ...
  · -- a_ge_b == 0
    ...
    suffices auto_spec : auto_spec_u32_overflowing_sub a b _ _ by
    ...
    simp only [htv_a_minus_b, u32Limit]
    exact ret_sub
  -- a_ge_b != 0
