...
theorem auto_sound_u32_overflowing_add
  ...
  (hmem : MemAt mem u32_overflowing_add_code σ.pc)
  ...
  : EnsuresRet mem σ (fun κ τ =>
    ∃ μ ≤ κ, RcEnsures mem (rcBound F) μ (mem (σ.fp - 5)) (mem (τ.ap - 3))
      (spec_u32_overflowing_add a b (mem (τ.ap - 2)) (mem (τ.ap - 1)))) := by
  ...
  · -- no_overflow = 0
    ...
    mkdef hl_fixed_a_plus_b : fixed_a_plus_b = temp_a_plus_b - u32Limit
    ...
    suffices auto_spec : auto_spec_u32_overflowing_add a b _ _ by
  ...
  have a9 : temp_fixed_a_plus_b = a_plus_b + (u128Limit - u32Limit) := by
