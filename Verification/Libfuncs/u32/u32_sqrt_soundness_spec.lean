...
def spec_u32_sqrt (a ρ_sqrt : F) : Prop := ∀ na, is_u32_of a na → is_u16_of ρ_sqrt na.sqrt

def auto_spec_u32_sqrt (a ρ_sqrt : F) : Prop :=
  ∃ ret_val : F,
...
theorem spec_u32_sqrt_of_auto_spec {a ρ_sqrt : F}
  (h_auto : auto_spec_u32_sqrt a ρ_sqrt) : spec_u32_sqrt a ρ_sqrt := by
  ...
  constructor
  · rw [← u16Limit_squared_eq] at hna_lt
  ...
  · have h_le : ret_val * ret_val ≤ na := by
      ...
      rw [PRIME.nat_coe_field_inj (lt_trans hna_lt u32Limit_lt_PRIME) _ h_lb]
      · exact le_add_self
    ...
    have h_ge : ret_val ≥ na.sqrt := by
      ...
      have h_right_lt : ub + na < PRIME := add_lt_PRIME_of_lt (lt_of_lt_of_le h_ub_lt h_rc_bound)
          (lt_trans hna_lt u32Limit_lt_u128Limit)
