...
theorem nextState_assert_eq :
  ...
  . show s.pc + instr.size = _ ↔ _
    ...
    cases ResSpec.toOp1 res <;>
      simp only [cond_true, cond_false, Nat.cast_zero, Nat.cast_one, zero_add, this, instr] <;> exact comm
  apply and_congr
...
theorem nextState_call (call_abs : Bool) :
  ...
  simp only [Instr.NextState, Instruction.NextState, instr]
  ...
  have hdst : instr.dst mem s = mem s.ap := by
    simp [callInstr, Instruction.dst, BitVec.toBiased16, intClip, natClip, instr]
  have hsize : s.pc + instr.size = bumpPc s res.toOp1.op1Imm := by
    simp [Instruction.nextPc, callInstr, Option.Agrees, Instruction.size, -ResSpec.toOp1, instr]
    have : (1 : F) + 1 = 2 := by norm_num
    cases ResSpec.toOp1 res <;>
  ...
  simp only [hsize, hdst, hop0, computeOp0, Int.cast_one, instr]
  norm_num [intClip_eq]
