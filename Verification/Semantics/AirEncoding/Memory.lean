...
/- The memory.  -/
open Classical in
noncomputable def mem (a' v' : Fin (n + 1) → F) : F → F := fun addr =>
...
include h_embed_mem_inj h_embed_dom h_embed_val in
theorem real_prod_eq :
  ...
  · apply Finset.prod_congr rfl
    ...
    simp [s, -not_exists, Finset.mem_image] at nsi
...
include h_embed_inst h_embed_mem_disj_inst in
...
include h_embed_dst_addr h_embed_mem_disj_dst in
...
include h_embed_dst h_embed_mem_disj_dst in
...
include h_embed_op0_addr h_embed_mem_disj_op0 in
...
include h_embed_op0 h_embed_mem_disj_op0 in
...
include h_embed_op1_addr h_embed_mem_disj_op1 in
...
include h_embed_op1 h_embed_mem_disj_op1 in
...
include h_embed_rc_addr h_embed_mem_disj_rc in
...
include h_embed_rc_val h_embed_mem_disj_rc in
...
include h_z_ne_zero h_embed_dom h_embed_val h_embed_mem_inj h_initial h_cumulative h_final
...
theorem real_permutation_prod_eq :
  ...
  suffices
    (∏ i, (z - (ra i + alpha * rv i))) * z ^ Fintype.card (MemDom mem_star) =
      (∏ i, (z - (a' i + alpha * v' i))) * z ^ Fintype.card (MemDom mem_star) by
...
include h_continuity h_single_valued h_char_lt
...
include h_embed_pc h_embed_inst h_embed_mem_disj_inst in
...
include h_embed_dst_addr h_embed_dst h_embed_mem_disj_dst in
...
include h_embed_op0_addr h_embed_op0 h_embed_mem_disj_op0 in
...
include h_embed_op1_addr h_embed_op1 h_embed_mem_disj_op1 in
...
include h_embed_rc_addr h_embed_rc_val h_embed_mem_disj_rc in
