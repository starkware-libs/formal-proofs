...
def on_ec {F : Type*} [field F] (p : F × F) := p.2^2 = p.1^3 + ALPHA * p.1 + BETA
...
def ec_double_slope (p : F × F) := (3 * p.1^2 + ALPHA) / (2 * p.2)
...
begin
  ...
  simp only [on_ec] at h1 h2,
  simp only [on_ec, ec_add, ec_add_slope],
  revert h1 h2,
  generalize : ALPHA = alpha,
  generalize : BETA = beta,
  intros h1 h2,
...
| ZeroPoint       := ZeroPoint
| (AffinePoint a) := AffinePoint ⟨a.x, -a.y, by { rw [on_ec, neg_sq], exact a.h } ⟩
