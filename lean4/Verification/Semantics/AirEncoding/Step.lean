...
-- from instruction constraints
variable (h_off_dst_tilde : off_dst_tilde = ↑inst.offDst.toNatr)
  (h_off_op0_tilde : off_op0_tilde = ↑inst.offOp0.toNatr)
  (h_off_op1_tilde : off_op1_tilde = ↑inst.offOp1.toNatr)
  (h_flags : ∀ i, f_tilde.toF i = ↑(inst.flags.get i).toNat)
...
theorem op1_agrees : (inst.op1 mem ⟨pc, ap, fp⟩).Agrees op1 := by
  have h1 : f_tilde.fOp1Imm = ↑inst.op1Imm.toNat := h_flags _
  have h2 : f_tilde.fOp1Ap = ↑inst.op1Ap.toNat := h_flags _
  have h3 : f_tilde.fOp1Fp = ↑inst.op1Fp.toNat := h_flags _
  ...
  cases inst.op1Imm <;> cases inst.op1Fp <;> cases inst.op1Ap <;>
    simp [Instruction.op1, Option.Agrees, Bitvec.toBiased16]
...
theorem resAux_agrees : f_tilde.fPcJnz = false.toNat → (inst.resAux mem ⟨pc, ap, fp⟩).Agrees res := by
  ...
  have h1 : f_tilde.fResAdd = ↑inst.resAdd.toNat := h_flags _
  have h2 : f_tilde.fResMul = ↑inst.resMul.toNat := h_flags _
...
theorem res_agrees : (inst.res mem ⟨pc, ap, fp⟩).Agrees res := by
  have h1 : f_tilde.fPcJnz = ↑inst.pcJnz.toNat := h_flags _
  have h2 :=
...
theorem nextPc_agrees : (inst.nextPc mem ⟨pc, ap, fp⟩).Agrees next_pc := by
  have h1 : f_tilde.fPcJumpAbs = ↑inst.pcJumpAbs.toNat := h_flags _
  have h2 : f_tilde.fPcJumpRel = ↑inst.pcJumpRel.toNat := h_flags _
  have h3 : f_tilde.fPcJnz = ↑inst.pcJnz.toNat := h_flags _
  have h4 : f_tilde.fDstReg = ↑inst.dstReg.toNat := h_flags _
  have h5 : f_tilde.fResMul = ↑inst.resMul.toNat := h_flags _
  have h6 : f_tilde.fResAdd = ↑inst.resAdd.toNat := h_flags _
  have h7 : f_tilde.fOp1Imm = ↑inst.op1Imm.toNat := h_flags _
...
theorem next_ap_agrees_aux (h : f_tilde.fOpcodeCall = 0) :
  ...
  have h1 : f_tilde.fApAdd = ↑inst.apAdd.toNat := h_flags _
  have h2 : f_tilde.fApAdd1 = ↑inst.apAdd1.toNat := h_flags _
  have h3 :=
    res_agrees h_off_op0_tilde h_off_op1_tilde h_flags h_mem_op0_addr h_mem_op1_addr h_op0_addr
      h_op1_addr h_mul h_res
  revert h_next_ap
...
theorem nextAp_agrees : (inst.nextAp mem ⟨pc, ap, fp⟩).Agrees next_ap := by
  have h1 : f_tilde.fOpcodeCall = ↑inst.opcodeCall.toNat := h_flags _
  have h3 : f_tilde.fApAdd = ↑inst.apAdd.toNat := h_flags _
  have h4 : f_tilde.fApAdd1 = ↑inst.apAdd1.toNat := h_flags _
  have h5 :=
...
theorem asserts_hold : inst.Asserts mem ⟨pc, ap, fp⟩ := by
  have h1 : f_tilde.fOpcodeCall = ↑inst.opcodeCall.toNat := h_flags _
  have h3 : f_tilde.fOpcodeAssertEq = ↑inst.opcodeAssertEq.toNat := h_flags _
  have h4 : f_tilde.fOp1Imm = ↑inst.op1Imm.toNat := h_flags _
...
theorem nextState_eq : NextState mem ⟨pc, ap, fp⟩ ⟨next_pc, next_ap, next_fp⟩ := by
  unfold NextState
  exact ⟨inst, h_mem_pc,
    nextPc_agrees h_off_dst_tilde h_off_op0_tilde h_off_op1_tilde h_flags
      ...
      h_mul h_res h_t0_eq h_t1_eq h_next_pc_eq h_next_pc_eq' h_opcode_assert_eq h_next_ap,
