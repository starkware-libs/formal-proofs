...
variable {F : Type _} [Field F] [Fintype F] {n : Type _} [Fintype n] (a b : n → F)
...
-- TODO: Why is the castSucc necessary?
theorem prod_sub_eq_zero_iff (z : F) : (∏ i : Fin n, (z - a i)) = 0 ↔ ∃ j : Fin n, z = a j := by
  ...
  exact (fun i => a (Fin.castSucc i))

variable [Fintype F]

-- TODO: Why the "exact a" in the lemma and final exact?
