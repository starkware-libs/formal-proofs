...
def ex1 : Fin 15 := @OfNat.ofNat (Fin 15) 4 Fin.instOfNatFin
-- @OfNat.ofNat (Fin 15) 4 Fin.instOfNatFin : Fin 15

theorem helper₁ (a b : ℕ) [NeZero b] : a < b → (@OfNat.ofNat (Fin b) a Fin.instOfNatFin).val = a := by
  exact Fin.val_cast_of_lt
  done

theorem helper₀ (a : ℕ) : ↑ex1 + a = 4 + a := by
...
def CpuOperands.toStepConstraints {c : Columns F} {inp : InputData F} (ops : CpuOperands c)
  ...
  { mul := c.cpuOperandsOpsMul (j * 16)
    ...
    h_op1Addr := by
      ...
      rw [sub_eq_zero] at h
      rw [← sub_eq_iff_eq_add.mpr h.symm]
      -- TODO: simp [helper₁] does nothing, but repeat rw does
      repeat rw [helper₁]
      ring
      -- Clear out the 3 < 15, etc. hypotheses from the lemma
      repeat norm_num1
    ...
    h_res := by
      ...
      repeat rw [helper₁]
      ring
      -- TODO: <;> norm_num1 doesn't work, but repeat does
      repeat norm_num1
    h_t0_eq := by
    ...
    h_opcode_call' := by
      ...
      dsimp
      simp only [TildeType.fOpcodeCall, TildeType.fOp1Imm, TildeType.toF, Column.off, add_zero,
    ...
    h_nextFp := by
      ...
      simp only [TildeType.fOpcodeCall, TildeType.fOpcodeRet, TildeType.toF, Column.off,
        Fin.val_succ, Fin.coe_castSucc, ← two_mul]
      norm_num
      repeat rw [helper₁]
      ring
...
def RcBuiltin.toRcBuiltinConstraints {inp : InputData F} {pd : PublicData F} {c : Columns F}
  ...
  h_rcValue := by
    ...
    simp only [Fin.val_zero, add_zero, Fin.val_one, Fin.val_two, add_mul, mul_assoc]
    norm_num; rfl
...
def PublicMemory.toMemoryEmbeddingConstraints {c : Columns F} {inp : InputData F}
  ...
  have jaux : ∀ j : Fin (inp.traceLength / 16 - 1), (j : ℕ) * 8 = 4 * (2 * j) := by
    intro j; rw [← mul_assoc, mul_comm (4 * 2)]
  { embedInst := fun i => by use (i.val) * 8 + 0; apply h 0 i; norm_num1
    ...
    h_embedMem_disj_inst := by
      ...
      simp only [Nat.mul_mod_right] at this
    h_embedMem_disj_dst := by
      intro a j h'
      rw [Fin.eq_iff_veq] at h' ; dsimp [embedMem] at h'
