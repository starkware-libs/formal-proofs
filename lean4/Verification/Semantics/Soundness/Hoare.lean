...
def MemAt (mem : F → F) : List F → F → Prop
  | [], _ => True
...
macro (name := arith_simps₁) "arith_simps" : tactic => `(tactic|
  --let simp_args : List simpArg := [Int.cast_zero, Int.cast_one, Int.cast_bit0, Int.cast_bit1, Int.cast_neg,
  --  Nat.cast_zero, Nat.cast_one, Nat.cast_bit0, Nat.cast_bit1, add_assoc,
  --  add_sub_assoc, add_zero, zero_add, add_right_neg, add_left_neg,
...
macro (name := ensures_simps₁) "ensures_simps" : tactic => `(tactic|
  ( try simp only [ResSpec.toOp1, computeRes, computeOp1, computeOp0, Op1Spec.op1Imm,
      computeDst, bumpPc, bumpAp, jumpPc, neg_clip_checked, clip_checked, checkedIntNz_eq,
...
-- TODO: Could refactor into a macro_rules and use an optional for h'
macro (name := step_assert_eq₁) "step_assert_eq" h:ident " with " hw:ident : tactic => `(tactic|
  ( ensuresb_make_succ
    arith_simps
...
macro (name := step_assert_eq₂) "step_assert_eq" h:ident h':ident " with " hw:ident : tactic => `(tactic|
  ( ensuresb_make_succ
    arith_simps
    apply assert_eq_ensuresb'
    apply $h
...
macro (name := step_jump) "step_jump" h:ident : tactic => `(tactic|
  ( ensuresb_make_succ
    apply jump_ensuresb
    apply $h
    clear $h
...
macro (name := step_jump_imm) "step_jump_imm" h:ident h':ident : tactic => `(tactic|
  ( ensuresb_make_succ
    apply jump_ensuresb'
    apply $h
    ensures_simps
...
macro (name := step_jnz₁) "step_jnz" h:ident " with " hw:ident hw':ident : tactic => `(tactic|
  ( ensuresb_make_succ
    apply jnz_ensuresb
    apply $h
    clear $h
...
macro (name := step_jnz₂) "step_jnz" h:ident h':ident " with " hw:ident hw':ident : tactic => `(tactic|
  ( ensuresb_make_succ
    apply jnz_ensuresb
    apply $h
    clear $h
...
macro (name := step_call) "step_call" h:ident : tactic => `(tactic|
  ( ensuresb_make_succ
    apply call_ensuresb
    apply $h
    clear $h
...
macro (name := step_ret) "step_ret" h:ident : tactic => `(tactic|
  ( ensuresb_make_succ
    apply ret_ensuresb
    apply $h
    clear $h
...
-- TODO: Refactor with an optional macro_rule, etc. see above
macro (name := step_advance_ap₁) "step_advance_ap" h:ident : tactic => `(tactic|
  ( ensuresb_make_succ
...
macro (name := step_advance_ap₂) "step_advance_ap" h:ident h':ident : tactic => `(tactic|
  ( ensuresb_make_succ
    apply advance_ap_ensuresb
    apply $h
    clear $h
...
/-
unsafe def step_sub (h1 : parse ident) (h2 : parse parser.pexpr) : tactic Unit :=
  focus1 do
    let eh ← get_local h1
    ensuresb_make_succ
...
unsafe def step_rec_sub (h1 : parse ident) (h2 : parse parser.pexpr) : tactic Unit :=
  focus1 do
    let eh ← get_local h1
    ensuresb_make_succ
...
/-
Experimental new versions. These expect terms rather than names of hypotheses in the local context,
and hence they cannot clear them.
-/
