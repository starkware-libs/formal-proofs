...
def addInstr (op0 : Op0Spec) (op1 : Op1Spec) (dst : DstSpec) (ap_update : Bool) := assertEqInstr op0 (ResSpec.op0_plus_op1 op1) dst ap_update

def add_instr (dst_base_fp op0_base_fp op1_imm op1_base_fp ap_update_add_1 offset0 offset1 offset2 : Felt) : Instr :=
    ...
    addInstr op0 op1 dst (ap_update_add_1 = 1)
    ...
    (ρCasmStateVal.strongly_bounded (num_steps+1) ∧ (∀ mem : Felt252 → Felt252,
      memory.Agrees mem →
        ∃ (offset0 offset1 offset2 dst_base_fp op0_base_fp op1_imm op1_base_fp ap_update_add_1 : Felt),
          mem (casmStateVal.pc.toFelt252) = (add_instr dst_base_fp op0_base_fp op1_imm op1_base_fp ap_update_add_1 offset0 offset1 offset2).toInstruction.toNat ∧
          (add_instr dst_base_fp op0_base_fp op1_imm op1_base_fp ap_update_add_1 offset0 offset1 offset2).toInstruction.NextState mem
            casmStateVal.toRegisterStateFelt252 ρCasmStateVal.toRegisterStateFelt252))

namespace AddSmallOpcode
...
def spec_auto
  ...
  ∃ (ρoffset0 ρoffset1 ρoffset2 : Felt) (ρflags : Fin 15 → Felt),
    DecodeInstruction.spec memory (none) (none) (none) (ADD_FLAGS) --
      casmStateVal.pc ρoffset0 ρoffset1 ρoffset2 ρflags ∧
    ∃ (dst op0 op1 dst_id op0_id op1_id: Felt),
      ((
            ReadSmall.spec memory
...
theorem spec_of_spec_auto [Fact (Nat.Prime Stwo.P)] [Fact (Nat.Prime Felt252Prime)]
    ...
    rcases h with ⟨memChecked, ρoffset0, ρoffset1, ρoffset2, ρflags, hdecode, dst, op0, op1,
        dst_id, op0_id, op1_id, ⟨hdst, hop0, hop1⟩, hop1flags, hoff2, h_add, rfl⟩
    ...
    dsimp [ADD_FLAGS, Flags.to_arr] at hflags
    ...
    · -- The instruction is in memory at pc
      ...
      dsimp [add_instr, assertEqInstr, addInstr] --
    ...
    · try rw [h_dst_fp] at hdst
      ...
      · try rw [h_op0_fp] at hop0
        ...
        try rw [← toFelt252_step_bounded_add_offset_eq ns_lim h_fp_nat fp_bound]
        try rw [← toFelt252_step_bounded_add_offset_eq ns_lim h_ap_nat ap_bound]
        have hdstmem := Felt252IdMemory.read_small_mem memChecked hmem hdst
...
def call
    ...
    let _state := DecodeInstruction.call airBuilder lookupTerms
                    none none none ADD_FLAGS casmState.pc
    let ab1 := _state.1
    let lt1 := _state.2.1
    ...
    let _state := ReadSmall.call ab6 lt1 (mem_dst_base + offset0)
    let ab7a := _state.1
    let lt2a := _state.2.1
    let dst := _state.2.2.1
    let _ := _state.2.2.2

    let _state := ReadSmall.call ab7a lt2a (mem0_base + offset1)
    let ab7b := _state.1
    let lt2b := _state.2.1
    let op0 := _state.2.2.1
    let _ := _state.2.2.2

    let _state := ReadSmall.call ab7b lt2b (mem1_base + offset2)
    let ab7 := _state.1
    let lt2 := _state.2.1
    let op1 := _state.2.2.1
    let _ := _state.2.2.2

    let ab8 := AirBuilder.constrain ab7 (dst - (op0 + op1))

    let next_ap := casmState.ap + flag_ap_update_add_1
    let next_pc := casmState.pc + FeltExpr.const 1 + flag_op1_imm
    (ab8, lt2, ⟨next_pc, next_ap, casmState.fp⟩)
...
theorem sound_auto [Fact (Nat.Prime Stwo.P)] [Fact (Nat.Prime Felt252Prime)] {t n_s : Nat}
  ...
  intro state1 ab1 lt1 offset0 offset1 offset2 flags
    ...
    next_pc
  intro hab8 lt2
  have ⟨hab7, h_add_pre⟩ := (AirBuilder.constrain_SatisfiedBy _ _ varAssign).mp hab8
  have ⟨hab7b, hlt2b, hread_op1⟩ := ReadSmall.sound_auto varAssign memAssign ab7b lt2b _ h_mem.1 (mem1_base + offset2) hab7 lt2
  have ⟨hab7a, hlt2a, hread_op0⟩ := ReadSmall.sound_auto varAssign memAssign ab7a lt2a _ h_mem.1 (mem0_base + offset1) hab7b hlt2b
  have ⟨hab6, hlt1, hread_dst⟩ := ReadSmall.sound_auto varAssign memAssign ab6 lt1 _ h_mem.1 (mem_dst_base + offset0) hab7a hlt2a
  ...
  have ⟨hab, hlt, h_decode⟩ := DecodeInstruction.sound_auto memAssign varAssign
    ab lt _ h_rc h_mem h_verify_instr none none none ADD_FLAGS casmState.pc hab1 hlt1
  ...
  constructor
  . exact h_decode
  use FeltExpr.eval varAssign dst
  use FeltExpr.eval varAssign op0
  use FeltExpr.eval varAssign op1

  have h_offset0_def : (DecodeInstruction.call ab lt none none none ADD_FLAGS casmState.pc).2.2.1 = offset0 := by
    rfl
  have h_offset1_def : (DecodeInstruction.call ab lt none none none ADD_FLAGS casmState.pc).2.2.2.1 = offset1 := by
    rfl
  have h_offset2_def : (DecodeInstruction.call ab lt none none none ADD_FLAGS casmState.pc).2.2.2.2.1 = offset2 := by
    rfl

  have h_flag_dst_base_fp_def: ((DecodeInstruction.call ab lt none none none ADD_FLAGS casmState.pc).2.2.2.2.2 FLAG_DST_BASE_FP_INDEX) = flag_dst_base_fp := by
    rfl
  have h_flag_op0_fp_def: ((DecodeInstruction.call ab lt none none none ADD_FLAGS casmState.pc).2.2.2.2.2 FLAG_OP0_BASE_FP_INDEX) = flag_op0_base_fp := by
    rfl
  have h_flag_op1_imm_def: ((DecodeInstruction.call ab lt none none none ADD_FLAGS casmState.pc).2.2.2.2.2 FLAG_OP1_IMM_INDEX) = flag_op1_imm := by
    rfl
  have h_flag_op1_fp_def: ((DecodeInstruction.call ab lt none none none ADD_FLAGS casmState.pc).2.2.2.2.2 FLAG_OP1_BASE_FP_INDEX) = flag_op1_base_fp := by
    rfl
  have h_flag_op1_ap_def: ((DecodeInstruction.call ab lt none none none ADD_FLAGS casmState.pc).2.2.2.2.2 FLAG_OP1_BASE_AP_INDEX) = flag_op1_base_ap := by
    rfl
  ...
  have hread_dst_simp : ReadSmall.spec memAssign (FeltExpr.eval varAssign mem_dst_base + FeltExpr.eval varAssign offset0) (FeltExpr.eval varAssign dst) (FeltExpr.eval varAssign dst_id) := by
    exact hread_dst
  have hread_op0_simp : ReadSmall.spec memAssign (FeltExpr.eval varAssign mem0_base + FeltExpr.eval varAssign offset1) (FeltExpr.eval varAssign op0) (FeltExpr.eval varAssign op0_id):= by
    exact hread_op0
  have hread_op1_simp : ReadSmall.spec memAssign (FeltExpr.eval varAssign mem1_base + FeltExpr.eval varAssign offset2) (FeltExpr.eval varAssign op1) (FeltExpr.eval varAssign op1_id) := by
    exact hread_op1
  ...
  constructor
  · simp only [FeltExpr.eval_sub] at h_add_pre
    have h_add := sub_eq_zero.mp h_add_pre
    simp only [FeltExpr.eval_add] at h_add
    exact h_add

  constructor

theorem sound_add_small [Fact (Nat.Prime Stwo.P)] [Fact (Nat.Prime Felt252Prime)] {t n_s : Nat}
      (varAssign : VarAssign)
      {memAssign : Felt252IdMemoryAssign}
      {mem : Felt252 → Felt252}
  ...
  use (add_instr dst_base_fp op0_base_fp op1_imm op1_base_fp ap_update_add_1 offset0 offset1 offset2).toInstruction
  use h_instr

end AddSmallOpcode

namespace Add252Opcode
...
-- (dst_base_fp op0_base_fp op1_imm op1_base_fp ap_update_add_1 : Bool) (offset0 offset1 offset2 : Felt)
...
theorem spec_of_spec_auto [Fact (Nat.Prime Stwo.P)] [Fact (Nat.Prime Felt252Prime)]
    ...
    rcases h with ⟨memChecked, ρoffset0, ρoffset1, ρoffset2, ρflags, hdecode, dst, op0, op1,
        ⟨hdst, hop0, hop1⟩, hop1flags, hoff2, h_add, rfl⟩
    ...
    · -- The instruction is in memory at pc
      ...
      · simp only [←BitVec_toFelt_inj, ←h_offOp1] --#check h_offOp1
        --cases op1Imm_val: instr.op1Imm
        · by_cases h_imm : ρflags 2 = 1
          · simp [h_imm]
            have := h_flags_imm h_imm
...
def call
    ...
    let lt2 := _state.2.1
    let op1 := _state.2.2
    let ab8 := VerifyAdd252.call ab7 op0 op1 dst
...
theorem sound_auto [Fact (Nat.Prime Stwo.P)] [Fact (Nat.Prime Felt252Prime)] {t n_s : Nat}
  ...
  have ⟨hab7, h_add_spec⟩ := VerifyAdd252.sound_auto varAssign ab7 op0 op1 dst hab8
  have ⟨hab7b, hlt2b, hread_op1⟩ := Felt252IdMemory.read_felt252.sound_auto varAssign memAssign ab7b _ _ h_rc h_mem.1 (mem1_base + offset2) hab7 hlt2
  ...
  have ⟨hab, hlt, h_decode⟩ := DecodeInstruction.sound_auto memAssign varAssign
    ab _ _ h_rc h_mem h_verify_instr none none none ADD_FLAGS casmState.pc hab1 hlt1
  ...
  · use dst_nats, op0_nats, op1_nats
    ...
    use h_op0_nats
    use h_op1_nats
    rw[h_add_spec op0_nats op1_nats dst_nats h_op0_nats h_op1_nats h_dst_nats]

  constructor

theorem sound_add_252 [Fact (Nat.Prime Stwo.P)] [Fact (Nat.Prime Felt252Prime)] {t n_s : Nat}
      (varAssign : VarAssign)
      {memAssign : Felt252IdMemoryAssign}
      {mem : Felt252 → Felt252}
      (h_mem_agrees : memAssign.Agrees mem)
