...
theorem intClip_eq {x : Int} (h₁ : -2 ^ 15 ≤ x) (h₂ : x < 2 ^ 15) : (intClip x : F) = x := by
  ...
  . conv => rhs; rw [h', Int.cast_sub, Int.cast_natCast]
    norm_num
  . apply Nat.lt_of_succ_le
    convert Nat.succ_le_succ h using 1

theorem intClip_eq' (x : Int) : intClip x = ((natClip x - 2 ^ 15 : Int) : F) := by
  norm_num [intClip]
...
theorem instruction_op1_eq (i : Instruction) (op0 : Op0Spec) (op1 : Op1Spec)
  ...
  cases op1 <;> simp [h, h', -computeOp0, BitVec.toBiased16, intClip] <;> norm_num
...
-- TODO(Jeremy): get rid of unused hypothesis
theorem instruction_res_op1_aux_eq (i : Instruction) (op0 : Op0Spec) (op1 : Op1Spec)
      (hop0: i.op0 mem s = computeOp0 mem s op0) -- ?
      (hop1: i.op1 mem s = some (computeOp1 mem s op0 op1)) :
    (match i.op1 mem s, (ResSpec.op1 op1).resAdd, (ResSpec.op1 op1).resMul with
      | some op1, false, false => some op1
      | some op1, true, false  => some (i.op0 mem s + op1)
      | some op1, false, true  => some (i.op0 mem s * op1)
      | _, _, _ => none)
    = some (computeRes mem s op0 (ResSpec.op1 op1)) := by
  rw [hop1]
  simp
...
theorem instruction_dst_eq (i : Instruction) (dst : DstSpec)
  ...
  cases dst <;> simp [Instruction.dst, h, h', BitVec.toBiased16, intClip] <;> norm_num
...
theorem nextState_assert_eq :
  ...
  have hop0 : instr.op0 mem s = computeOp0 mem s op0 := by
    apply instruction_op0_eq <;> simp [assertEqInstr, instr]
  have hop1 : instr.op1 mem s = computeOp1 mem s op0 res.toOp1 := by
    have := instruction_op1_eq (h := hop0)
    apply instruction_op1_eq (h := hop0); simp [assertEqInstr, instr]
  ...
  have hdst : instr.dst mem s = computeDst mem s dst := by
    apply instruction_dst_eq <;> simp [assertEqInstr, instr]
  apply and_congr
...
theorem nextState_jump (jump_abs : Bool) :
  ...
  have hop0 : instr.op0 mem s = computeOp0 mem s op0 := by
    apply instruction_op0_eq <;> simp [jumpInstr, instr]
  have hop1 : instr.op1 mem s = computeOp1 mem s op0 res.toOp1 := by
    apply instruction_op1_eq (h := hop0); simp [jumpInstr, instr]
  have hres : instr.res mem s = some (computeRes mem s op0 res) := by
...
theorem nextState_jnz (op1 : Op1Spec) :
  ...
  have hop0 : instr.op0 mem s = computeOp0 mem s op0 := by
    apply instruction_op0_eq <;> simp [jnzInstr, instr]
  have hop1 : instr.op1 mem s = computeOp1 mem s op0 op1 := by
    apply instruction_op1_eq (h := hop0); simp [jnzInstr, instr]
  have hdst : instr.dst mem s = computeDst mem s dst := by
    apply instruction_dst_eq <;> simp [jnzInstr, instr]
  have hsize : s.pc + instr.size = bumpPc s op1.op1Imm := by
    have : (1 : F) + 1 = 2 := by norm_num
    cases ap_update <;> simp [Instruction.size, jnzInstr, instr] <;> cases op1 <;> simp [this]
  apply and_congr
...
theorem nextState_call (call_abs : Bool) :
  ...
  have hop0 : instr.op0 mem s = computeOp0 mem s (Op0Spec.ap_plus 1) := by
    apply instruction_op0_eq <;> simp [callInstr, instr]
  have hop1 : instr.op1 mem s = computeOp1 mem s (Op0Spec.ap_plus 1) res.toOp1 := by
    apply instruction_op1_eq (h := hop0); simp [callInstr, instr]
  ...
  have hdst : instr.dst mem s = mem s.ap := by
    simp [callInstr, Instruction.dst, BitVec.toBiased16, natClip, instr]
  have hsize : s.pc + instr.size = bumpPc s res.toOp1.op1Imm := by
    simp [callInstr, Instruction.size, -ResSpec.toOp1, instr]
    have : (1 : F) + 1 = 2 := by norm_num
    cases ResSpec.toOp1 res <;>
      simp only [cond_true, cond_false, Nat.cast_zero, Nat.cast_one, zero_add, this]
  apply and_congr
  . cases call_abs
    . show Option.Agrees
  ...
  . simp [Instruction.nextAp, Instruction.nextApAux, Option.Agrees]
    exact comm
  apply and_congr
  . simp [Instruction.nextFp, callInstr, Option.Agrees]; exact comm
  show instr.op0 mem s = s.pc + instr.size ∧ instr.dst mem s = s.fp ↔ _
  simp only [hsize, hdst, hop0, computeOp0, instr]
  norm_num [intClip_eq]
...
theorem nextState_ret :
  ...
  simp [Instr.NextState, retInstr, Instruction.NextState, Option.Agrees, Instruction.nextPc,
    Instruction.nextFp, Instruction.nextAp, Instruction.Asserts, Instruction.dst,
...
theorem nextState_advance_ap :
  ...
  have hop0 : instr.op0 mem s = computeOp0 mem s op0 := by
    apply instruction_op0_eq <;> simp [advanceApInstr, instr]
  have hop1 : instr.op1 mem s = computeOp1 mem s op0 res.toOp1 := by
    apply instruction_op1_eq (h := hop0); simp [advanceApInstr, instr]
  have hres : instr.res mem s = some (computeRes mem s op0 res) := by
    apply instruction_res_aux_eq (hop0 := hop0) (hop1 := hop1)
  apply and_congr
  . show s.pc + instr.size = _ ↔ _
    simp [Instruction.size, advanceApInstr, -ResSpec.toOp1, instr]
    have : (1 : F) + 1 = 2 := by norm_num
    cases ResSpec.toOp1 res <;>
      simp only [cond_true, cond_false, Nat.cast_zero, Nat.cast_one, zero_add, this] <;> exact comm
  apply and_congr
