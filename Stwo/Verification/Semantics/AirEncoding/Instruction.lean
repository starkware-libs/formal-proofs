...
def tilde (i : Fin (n + 1)) : ℕ :=
  ∑ j ∈ i.revPerm.range, (2 ^ ((Fin.castSucc (Fin.revPerm j)) - i : ℕ) * (b.getLsbD (Fin.revPerm j)).toNat)
...
theorem tilde_zero_eq : b.tilde 0 = b.toNat := by
  ...
  have hb : b = b'.concat (b.getLsbD 0) := by
    ...
    intro i hi; rcases i with _ | i
    . rw [getLsbD_concat_zero]; rfl
    . rw [getLsbD_concat_succ]
      change _ = BitVec.getLsbD _ ↑(Fin.mk i (Nat.lt_of_succ_lt_succ hi))
      rw [getLsbD_ofFnLE]; rfl
  conv =>
    rhs
...
theorem fromTilde_tilde : fromTilde b.tilde = b := by
  ...
  apply BitVec.eq_of_getLsbD_eq
  intro i h
  change BitVec.getLsbD _ ↑(Fin.mk i h) = _
...
theorem toNat_inj {i1 i2 : Instruction} (h : i1.toNat = i2.toNat) : i1 = i2 := by
  ...
  have h' := BitVec.toNat_inj.mp h
  apply
    Instruction.ext (BitVec.toNat_inj.mp h1) (BitVec.toNat_inj.mp h2) (BitVec.toNat_inj.mp h3)
...
theorem theInstruction_flags_eq : (theInstruction h_bit offOp0_in_range offOp1_in_range offDst_in_range).flags =
  ...
  convert BitVec.ofBoolListLE_cast _ _ rfl _ <;> simp
