...
def a'NatOffset (a' : Fin (n + 1) → F) (i : Fin (n + 1)) : ℕ := ∑ j ∈ Fin.range i, (a'Step a') j
...
theorem permutation_aux (j : Fin (n + 1)) :
    (∏ i ∈ Fin.range j.succ, (z - (a i + alpha * v i))) =
      (∏ i ∈ Fin.range j.succ, (z - (a' i + alpha * v' i))) * p j := by
