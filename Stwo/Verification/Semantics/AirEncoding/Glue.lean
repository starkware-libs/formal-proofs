...
def CpuOperands.toStepConstraints {c : Columns F} {inp : InputData F} (ops : CpuOperands c)
  ...
  { mul := c.cpuOperandsOpsMul (j * 16)
    ...
    h_op0Addr := by
      ...
      ring
    h_op1Addr := by
      simp [TildeType.fOp1Imm, TildeType.fOp1Ap, TildeType.fOp1Fp, TildeType.toF, Column.off, two_mul, Fin.coe_ofNat_eq_mod]
    ...
    h_res := by
      simp [TildeType.fPcJnz, TildeType.fResAdd, TildeType.fResMul, TildeType.toF, Column.off, two_mul, Fin.coe_ofNat_eq_mod]
      have h := ops.res _ h0
      rw [sub_eq_zero] at h
      trans
      apply h
    ...
    h_nextPc_eq' := by
      ...
      simp [TildeType.fPcJnz, TildeType.fPcJumpAbs, TildeType.fPcJumpRel, TildeType.fOp1Imm,
        TildeType.toF, Column.off, two_mul, add_zero, add_mul, Fin.coe_ofNat_eq_mod]
      ring
    h_opcode_call := by
    ...
    h_nextAp := by
      ...
      simp only [TildeType.fOpcodeCall, TildeType.fApAdd, TildeType.fApAdd1, TildeType.toF,
        Column.off, Fin.val_succ, Fin.coe_castSucc, ← two_mul, Fin.coe_ofNat_eq_mod]
      norm_num
      ring
    h_nextFp := by
      dsimp
...
def Memory.toMemoryBlockConstraints {inp : InputData F} {pd : PublicData F} {c : Columns F}
  ...
  { a' := fun j => c.column20 (2 * j)
    ...
    h_single_valued := by
      ...
      simp [Column.off, add_mul, mul_comm 2, add_assoc]
      ring
    h_initial := by
...
def RcBuiltin.toRcBuiltinConstraints {inp : InputData F} {pd : PublicData F} {c : Columns F}
  ...
  h_rcValue := by
    ...
    simp [add_zero, add_mul, mul_assoc, Column.off, Fin.coe_ofNat_eq_mod]
...
def toConstraints {inp : InputData F} {pd : PublicData F} {c : Columns F} {ci : ColumnsInter F}
  ...
  { fp := fun j => c.cpuRegistersFp (j * 16)
    ...
    h_pcF := by rw [mul_comm _ 16]; exact eq_of_sub_eq_zero (iandf.finalPc _ rfl)
    h_apF := by rw [mul_comm _ 16]; exact eq_of_sub_eq_zero (iandf.finalAp _ rfl)
    mc :=
