...
theorem vmNextState_assert_eq :
  ...
  . show s.pc + instr.vmSize = _ ↔ _
    simp [assertEqInstr, -ResSpec.toOp1, instr]
    cases ResSpec.toOp1 res <;> exact comm
  apply and_congr
  . simp [assertEqInstr]
    cases ap_update <;> simp <;> exact comm
  apply and_congr
  .  simp [assertEqInstr]; exact comm
...
theorem vmNextState_jnz (op1 : Op1Spec) :
  ...
  apply and_congr
  . simp [Instruction.vmNextAp, Instruction.vmNextApAux]
    cases ap_update <;> simp <;> exact comm
  simp [Instruction.vmNextFp, Instruction.VmAsserts, jnzInstr]; exact comm
...
theorem vmNextState_call (call_abs : Bool) :
  ...
  apply and_congr
  swap
  · simp only [callInstr, Option.Agrees, Instruction.vmSize, Instruction.vmNextFp,
      Instruction.vmNextAp, Instruction.VmAsserts, Instruction.vmDst, Instruction.vmNextApAux]
    apply and_congr
    · simp ; constructor <;> intro h <;> rw [h]
    apply and_congr
    · simp ; constructor <;> intro h <;> rw [h]
    apply and_congr
    · simp only [Instruction.vmOp0]
...
theorem vm_next_state_ret :
  ...
  simp [Instr.VmNextState, retInstr, Instruction.VmNextState, Option.Agrees,
    Instruction.vmNextPc, Instruction.vmNextFp, Instruction.vmNextAp,
...
theorem vm_next_state_advance_ap :
  ...
  · have hsize : s.pc + instr.vmSize = vmBumpPc s res.toOp1.op1Imm := by
      have h_op1Imm : (advanceApInstr op0 res).op1Src.fst = res.toOp1.op1Imm := by
        simp [advanceApInstr]
  ...
  · have hop0 : instr.vmOp0 mem s = vmComputeOp0 mem s op0 := by
    ...
    simp only [hres]
    constructor <;> apply Eq.symm
  simp [advanceApInstr, Option.Agrees, Instruction.vmNextFp, Instruction.VmAsserts]
  constructor <;> apply Eq.symm
