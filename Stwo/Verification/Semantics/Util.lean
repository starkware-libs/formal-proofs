...
attribute [int_cast_simps] Int.ofNat_zero Int.ofNat_one Int.ofNat_add
  WithZero.coe_pow Int.cast_add Int.cast_mul Int.cast_sub Int.cast_zero
  Int.cast_one
...
theorem castSucc_add_one_revPerm {n : ℕ} {i : Fin n} :
  ...
  · apply Nat.succ_lt_succ_iff.mpr
    cases' n with n
    · exact absurd (Nat.zero_le i') (not_le_of_gt h')
...
@[simp]
theorem range_zero : range (0 : Fin (n + 1)) = ∅ := by ext i; simp [range]
...
theorem sum_range_one (f : Fin (n + 1) → β) : (∑ k ∈ range 1, f k) = f 0 := by
  rw [range_one, Finset.sum_singleton]

theorem sum_range_zero (f : Fin n → β) : (∑ k ∈ range 0, f k) = 0 :=
  by rw [range_zero, Finset.sum_empty]

theorem sum_range_succ' (f : Fin (n + 1) → β) (i : Fin (n + 1)) :
    (∑ k ∈ range i.succ, f k) = (∑ k ∈ range i, f k.succ) + f 0 := by
  induction' i using Fin.inductionOn with i' ih
...
theorem ofFnBE_getMsbD : (ofFnBE fun i : Fin n => getMsbD b i) = b := by
  apply eq_of_getMsbD_eq; intro i h
  change getMsbD _ ↑(Fin.mk i h) = _
  rw [getMsbD_ofFnBE (fun i : Fin n => getMsbD b i)]
...
theorem getLsbD_ofFnLE (f : Fin n → Bool) (i : Fin n) :
  ...
  rw [ofFnLE, getLsbD_cast, BitVec.getLsbD_ofBoolListLE]
  simp

theorem ofFnLE_getLsb : (ofFnLE fun i : Fin n => getLsbD b i) = b := by
  apply eq_of_getLsbD_eq; intro i h
  change getLsbD _ ↑(Fin.mk i h) = _
  rw [getLsbD_ofFnLE]
